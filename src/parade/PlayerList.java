...
public class PlayerList {
        ...
        for (int i = 0; i < HAND_SIZE; i++) {
            for (Player p : playerList) {
                try {
                    ...
                    // if printWinner is going to be called when catching endgame exception
                    // shouldn't this be a different exception
    ...
    // in player class,
    // collected cards per player rep by EnumMap<Colour, ArrayList<Card>>
    // collectedCards
    // use getCollectedCards
    // to remove cards from collectedCards
    private void calculateScores() { // should only be called once

        // EnumMap<Colour, ArrayList<Player>> AllColourMaxPlayers = new
        // EnumMap<>(Colour.class);
        // iter through colours
        for (Colour colour : Colour.values()) {
            ArrayList<Player> maxPlayers = findMaxPlayers(colour); // get list of players with the maximum number of
                                                                   // that card
        ...
        addRemainingFaces();
        // Player winner = Collections.min(playerList,
        // Comparator.comparingInt(Player::getScore));
        // String winnerName= "Player " + (playerList.indexOf(winner) + 1);
        // System.out.println(winnerName + " wins with a score of " + winner.score +
        // "!"); // can use winner.getName() instead if getname implemented
    }
    ...
    private ArrayList<Player> findMaxPlayers(Colour colour) {
        ...
        for (Player p : playerList) {
            ...
            // if 2 players, get player with majority of >= 2
                ...
                // P1 might have been wrongly added bc size was compared to initial maxCount of
                // 0
                if (maxCount - count < 2) {
                    maxPlayers.clear();
                }
